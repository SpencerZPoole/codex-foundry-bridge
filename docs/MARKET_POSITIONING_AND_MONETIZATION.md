...
5. Package setup for real users: installer, credential wizard polish, compatibility matrix, first-run self-check, screenshot workflow examples, and one-click disposable-world validation.
6. Produce a public demo that shows the bridge preventing a bad edit, previewing a good edit, applying it with confirmation, verifying the resulting Foundry state, and capturing clean visual evidence from the live app.
